...
const OWN_ACCOUNTS = new Set(["SBI", "MIZUHO", "GMO", "GA_ESOP"]);
const KIDS_ACCOUNTS = new Set(["SBI_DAUGHTER", "SBI_DAUGHTER_YOUNGER"]);
...
function renderReview(data) {
  ...
  renderJpPicks(data);
  renderEquityResearch(data);
  renderSkylarkBenefit(data);
}

function renderSensitivityTable(sens) {
  if (!sens || !sens.matrix_implied_price || !sens.matrix_implied_price.length) return "";
  const rows = sens.row_values || [];
  const cols = sens.col_values || [];
  const matrix = sens.matrix_implied_price;
  const header = `<tr><th>${escapeHtml(sens.row_label || "")} \\ ${escapeHtml(sens.col_label || "")}</th>${cols
    .map((c) => `<th>${formatPctLabel(c, 2)}</th>`)
    .join("")}</tr>`;
  const body = matrix
    .map(
      (line, i) =>
        `<tr><th>${formatPctLabel(rows[i], 2)}</th>${(line || [])
          .map((cell, j) => {
            const center = i === 2 && j === 2;
            return `<td class="${center ? "sens-center" : ""}">${Math.round(Number(cell) || 0).toLocaleString("ja-JP")}</td>`;
          })
          .join("")}</tr>`
    )
    .join("");
  return `<div class="table-scroll"><table class="sens-table">${header}${body}</table></div>`;
}

function renderEquityResearch(data) {
  const block = $("equity-research-block");
  if (!block) return;
  const payload = data.equity_research || {};
  const note = $("equity-research-note");
  const body = $("equity-research-body");
  const disc = $("equity-research-disclaimer");
  const stamp = formatReviewStamp(payload.generated_at || "");
  if (note) {
    note.textContent = [
      payload.reference ? "Anthropic FS 参考" : "",
      payload.tickers && payload.tickers.length ? payload.tickers.join(", ") : "",
      stamp,
    ]
      .filter(Boolean)
      .join("  ·  ");
  }
  if (disc) disc.textContent = payload.disclaimer || "";
  if (!body) return;
  const reports = payload.reports || [];
  if (!reports.length) {
    body.innerHTML = `<p class="muted">次回の更新で DCF / リサーチドラフトを表示します。</p>`;
    return;
  }
  body.innerHTML = reports
    .map((item) => {
      const dcf = item.dcf || {};
      const er = item.equity_research || {};
      const bridge = dcf.valuation_bridge || {};
      const sens = renderSensitivityTable(dcf.sensitivity);
      const cites = [...(dcf.citations || []), ...(er.citations || [])]
        .filter((cite, idx, arr) => arr.findIndex((x) => x.url === cite.url) === idx)
        .map(
          (cite) =>
            `<li><a href="${escapeHtml(cite.url || "")}" target="_blank" rel="noopener noreferrer">${escapeHtml(
              cite.title || cite.url || ""
            )}</a></li>`
        )
        .join("");
      return `<article class="insight-card jp-pick-card">
        <h3>${escapeHtml(item.name || item.ticker || "")} <small>${escapeHtml(item.ticker || "")}</small></h3>
        <p class="muted"><strong>DCF</strong> · ${escapeHtml(dcf.executive_summary || "")}</p>
        <div class="holding-metrics">
          <div><span>DCF参考株価</span><strong>${yen(bridge.implied_share_price)}</strong></div>
          <div><span>現在株価</span><strong>${yen(bridge.current_price)}</strong></div>
          <div><span>モデル差</span><strong>${formatPctLabel(bridge.upside_pct, 1)}</strong></div>
          <div><span>WACC / g</span><strong>${formatPctLabel((dcf.assumptions || {}).wacc_pct, 1)} / ${formatPctLabel((dcf.assumptions || {}).terminal_growth_pct, 1)}</strong></div>
        </div>
        ${sens}
        <p><strong>Research headline</strong> ${escapeHtml(er.headline || "")}</p>
        <p><strong>Thesis</strong> ${escapeHtml(er.investment_thesis || "")}</p>
        <p><strong>Bull / Bear</strong> ${escapeHtml(er.bull_case || "")} / ${escapeHtml(er.bear_case || "")}</p>
        <p><strong>Valuation view</strong> ${escapeHtml(er.valuation_view || "")}</p>
        <p><strong>Catalysts</strong> ${escapeHtml(er.catalysts || "")}</p>
        <p><strong>Monitor</strong> ${escapeHtml(er.monitoring_points || "")}</p>
        ${cites ? `<ul class="jp-deep-cites">${cites}</ul>` : ""}
      </article>`;
    })
    .join("");
}
...
function renderJpDeepDive(dive) {
  if (!dive || !dive.company_overview) return "";
  const macro = dive.macro_environment || {};
  const cites = (dive.citations || [])
    .map(
      (cite) =>
        `<li><a href="${escapeHtml(cite.url || "")}" target="_blank" rel="noopener noreferrer">${escapeHtml(
          cite.title || cite.url || ""
    ...
    .join("");
  return `<section class="jp-deep-dive">
    <h4>詳細分析（エクイティ・リサーチ）</h4>
    <p><strong>会社概要</strong> ${escapeHtml(dive.company_overview || "")}</p>
    <p><strong>マクロ（日本）</strong> 追い風: ${escapeHtml(macro.tailwind_jp || "")} / 向かい風: ${escapeHtml(
      macro.headwind_jp || ""
    )}</p>
    <p><strong>マクロ（米国）</strong> 追い風: ${escapeHtml(macro.tailwind_us || "")} / 向かい風: ${escapeHtml(
      macro.headwind_us || ""
    )}</p>
    <p><strong>財務状況</strong> ${escapeHtml(dive.financials || "")}</p>
    <p><strong>同業比較・バリュエーション</strong> ${escapeHtml(dive.peer_valuation || "")}</p>
    <p><strong>中期計画・アナリスト視点</strong> ${escapeHtml(dive.midterm_and_analyst_view || "")}</p>
    <p><strong>テクニカル</strong> ${escapeHtml(dive.technical_analysis || "")}</p>
    <p><strong>注目水準</strong> ${escapeHtml(dive.watch_levels || "")}</p>
    ${cites ? `<ul class="jp-deep-cites">${cites}</ul>` : ""}
  </section>`;
}

function renderJpPickSummary(item, recommendation) {
  const rec = recommendation || {};
  return `<article class="insight-card jp-pick-card jp-pick-featured">
    <h3>${escapeHtml(item.name || "")}（${escapeHtml(item.code || "")}）</h3>
    <p class="muted">${escapeHtml(item.sector || "")} ${escapeHtml(item.industry || "")} · 総合点 ${escapeHtml(
      String(item.score ?? "—")
    )}</p>
    <p><strong>おすすめ理由</strong> ${escapeHtml(rec.rationale || item.appeal || "")}</p>
    <p><strong>他候補より優先</strong> ${escapeHtml(rec.why_over_others || "")}</p>
    <p><strong>取得検討条件</strong> ${escapeHtml(rec.entry_conditions || "")}</p>
    <p><strong>最大リスク</strong> ${escapeHtml(rec.key_risk || item.max_risk || "")}</p>
    <p><strong>なぜ割安か</strong> ${escapeHtml(item.why_cheap || "")}</p>
    <p><strong>なぜ成長が続くか</strong> ${escapeHtml(item.why_growth || "")}</p>
    <p><strong>10倍シナリオ</strong> ${escapeHtml(item.tenbagger_scenario || "")}</p>
    ${renderJpDeepDive(item.deep_dive || {})}
  </article>`;
}

function renderJpPicks(data) {
  const block = $("jp-picks-block");
  if (!block) return;
  const payload = data.jp_growth_picks || {};
  const picks = payload.picks || [];
  const recommendation = payload.recommendation || {};
  const recommended = payload.recommended_pick || null;
  const recommendedTicker = recommendation.recommended_ticker || recommended?.ticker || "";
  const note = $("jp-picks-note");
  const table = $("jp-picks-table");
  const recBlock = $("jp-picks-recommended-block");
  const recEl = $("jp-picks-recommended");
  const xEl = $("jp-picks-x");
  const disc = $("jp-picks-disclaimer");
  const stamp = formatReviewStamp(payload.generated_at || "");
  const headline = payload.headline || "有望日本株 Pickup";
  if (note) {
    note.textContent = [headline, stamp, payload.note].filter(Boolean).join("  ·  ");
  }
  if (disc) disc.textContent = payload.disclaimer || "";
  if (!picks.length) {
    if (recBlock) recBlock.hidden = true;
    if (recEl) recEl.innerHTML = "";
    if (table) table.innerHTML = `<p class="muted">該当銘柄なし。次回の朝更新で再調査します。</p>`;
    if (xEl) {
      xEl.hidden = true;
      xEl.textContent = "";
    }
  ...
  if (recBlock && recEl && recommended) {
    recBlock.hidden = false;
    recEl.innerHTML = renderJpPickSummary(recommended, recommendation);
  } else if (recBlock) {
    recBlock.hidden = true;
  }
  if (table) {
    table.innerHTML = picks
      .map((item) => {
        const pe = item.forward_pe != null ? item.forward_pe : item.trailing_pe;
        const peLabel = pe != null ? `${Number(pe).toFixed(1)}倍` : "—";
        const held = item.held ? `<span class="jp-held">保有</span>` : "";
        const featured =
          recommendedTicker && item.ticker === recommendedTicker
            ? `<span class="jp-held">おすすめ</span>`
            : "";
        const url = item.yahoo_url || `https://finance.yahoo.co.jp/quote/${escapeHtml(item.ticker || "")}`;
        const name = escapeHtml(item.name || item.ticker || "");
        return `<article class="holding-card jp-pick-row">
          <div>
            <a class="holding-name" href="${escapeHtml(url)}" target="_blank" rel="noopener noreferrer">${item.rank}. ${name}</a>
            <small class="holding-ticker">${escapeHtml(item.code || item.ticker || "")} ${held} ${featured}</small>
          </div>
          <div class="holding-metrics">
            <div><span>株価</span><strong>${yen(item.price)}</strong></div>
            <div><span>予想PER</span><strong>${peLabel}</strong></div>
            <div><span>売上成長</span><strong>${formatPctLabel(item.revenue_growth_pct)}</strong></div>
            <div><span>営利成長</span><strong>${formatPctLabel(item.op_growth_pct)}</strong></div>
            <div><span>総合点</span><strong>${item.score != null ? item.score : "—"}</strong></div>
          </div>
        </article>`;
      })
      .join("");
  ...
  if (xEl) {
    xEl.hidden = !payload.x_post;
    xEl.innerHTML = payload.x_post
      ? `<p class="muted">X投稿用</p><pre class="x-post">${escapeHtml(payload.x_post)}</pre>`
      : "";
  }
}

function renderSkylarkBenefit(data) {
  const block = $("skylark-benefit-block");
  if (!block) return;
  const payload = data.skylark_benefit || {};
  const analysis = payload.analysis || {};
  const note = $("skylark-benefit-note");
  const body = $("skylark-benefit-body");
  const disc = $("skylark-benefit-disclaimer");
  const stamp = formatReviewStamp(payload.generated_at || "");
  const cost = payload.cost_100_jpy != null ? yen(payload.cost_100_jpy) : "—";
  if (note) {
    note.textContent = [
      `3197 すかいらーく · 目標 ${payload.target_shares || 100}株`,
      `必要資金目安 ${cost}`,
      stamp,
    ]
      .filter(Boolean)
  ...
  if (!analysis.company_overview) {
    body.innerHTML = `<p class="muted">次回の朝更新で表示します。</p>`;
    return;
  }
  const cites = (analysis.citations || [])
    .map(
    ...
    .join("");
  body.innerHTML = `<article class="insight-card jp-pick-card">
    <p><strong>会社概要</strong> ${escapeHtml(analysis.company_overview || "")}</p>
    <p><strong>事業・財務の見方</strong> ${escapeHtml(analysis.investment_case || "")}</p>
    <p><strong>株主優待（100株）</strong> ${escapeHtml(analysis.benefit_summary || "")}</p>
    <p><strong>優待価値の目安</strong> ${escapeHtml(analysis.benefit_value_estimate || "")}</p>
    <p><strong>取得検討条件</strong> ${escapeHtml(analysis.purchase_conditions || "")}</p>
    <p><strong>タイミング</strong> ${escapeHtml(analysis.timing_note || "")}</p>
    <p><strong>リスク</strong> ${escapeHtml(analysis.risks || "")}</p>
    ${cites ? `<ul class="jp-deep-cites">${cites}</ul>` : ""}
  </article>`;
}
...
function isLiveQuoteEligible(item) {
  if (!item || !item.ticker) return false;
  if (item.asset_type === "CASH" || item.asset_type === "FUND") return false;
  if (item.price_status === "MANUAL") return false;
  if (String(item.price_source || "").toUpperCase() === "MANUAL") return false;
  return true;
}

function recalcPortfolioMetrics(data) {
  const holdings = data.holdings || [];
  const sum = (pred, field) =>
    holdings.filter(pred).reduce((total, item) => total + (Number(item[field]) || 0), 0);
  const totalValue = sum(() => true, "market_value_jpy");
  holdings.forEach((item) => {
    const mv = Number(item.market_value_jpy) || 0;
    item.portfolio_weight_pct = totalValue ? (mv / totalValue) * 100 : null;
  });
  const ownValue = sum((item) => OWN_ACCOUNTS.has(item.account), "market_value_jpy");
  const kidsValue = sum((item) => KIDS_ACCOUNTS.has(item.account), "market_value_jpy");
  const totalPnl = sum(() => true, "unrealized_pnl_jpy");
  const totalDaily = sum(() => true, "daily_change_jpy");
  const ownPnl = sum((item) => OWN_ACCOUNTS.has(item.account), "unrealized_pnl_jpy");
  const ownDaily = sum((item) => OWN_ACCOUNTS.has(item.account), "daily_change_jpy");
  const totalCost = totalValue - totalPnl;
  const ownCost = ownValue - ownPnl;
  const p = data.portfolio || {};
  p.market_value_total = totalValue;
  p.market_value_own = ownValue;
  p.market_value_kids = kidsValue;
  p.market_value = totalValue;
  p.unrealized_pnl = totalPnl;
  p.unrealized_pnl_pct = totalCost ? (totalPnl / totalCost) * 100 : null;
  p.daily_change = totalDaily;
  const priorTotal = totalValue - totalDaily;
  p.daily_change_pct = priorTotal ? (totalDaily / priorTotal) * 100 : null;
  p.unrealized_pnl_own = ownPnl;
  p.unrealized_pnl_pct_own = ownCost ? (ownPnl / ownCost) * 100 : null;
  p.daily_change_own = ownDaily;
  const priorOwn = ownValue - ownDaily;
  p.daily_change_pct_own = priorOwn ? (ownDaily / priorOwn) * 100 : null;
  data.portfolio = p;
  if (data.headline) {
    data.headline.daily_change_jpy = p.daily_change_own;
    data.headline.daily_change_pct = p.daily_change_pct_own;
    data.headline.unrealized_pnl_jpy = p.unrealized_pnl_own;
    data.headline.unrealized_pnl_pct = p.unrealized_pnl_pct_own;
  }
}
...
async function refreshLiveQuotes() {
  ...
  const liveable = (data.holdings || []).filter(isLiveQuoteEligible);
  ...
  recalcPortfolioMetrics(data);
  if (data.headline) data.headline.usd_jpy = usdJpy;
...
function render(data) {
  ...
  const p = data.portfolio || {};
  $("sec-value").textContent = yen(p.market_value_total ?? p.market_value);
  $("sec-pnl").textContent = `${signedYen(p.unrealized_pnl ?? p.unrealized_pnl_own)}  ${pct(
    p.unrealized_pnl_pct ?? p.unrealized_pnl_pct_own
  )}`;
  $("sec-pnl").className = "hero-sub " + toneClass(p.unrealized_pnl ?? p.unrealized_pnl_own);
  const kidsEl = $("sec-kids");
  if (kidsEl) {
    const ownVal = p.market_value_own;
    const kidsValue = p.market_value_kids;
    const parts = [];
    if (ownVal != null && ownVal > 0) parts.push(`本人 ${yen(ownVal)}`);
    if (kidsValue != null && kidsValue > 0) parts.push(`ジュニアNISA ${yen(kidsValue)}`);
    kidsEl.textContent = parts.length ? parts.join("  ·  ") : "";
  }
