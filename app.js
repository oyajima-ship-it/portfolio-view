...
function renderJpPicks(data) {
      ...
      .map((item) => {
        ...
        return `<article class="holding-card jp-pick-row">
          <div>
            ...
            <small class="holding-ticker">${escapeHtml(item.code || item.ticker || "")} ${held}</small>
          </div>
  ...
  if (detail) {
    detail.innerHTML = picks
      .map((item) => {
        return `<article class="insight-card jp-pick-card">
          <h3>${escapeHtml(item.rank)}. ${escapeHtml(item.name || "")}（${escapeHtml(item.code || "")}）</h3>
          <p class="muted">${escapeHtml(item.sector || "")} ${escapeHtml(item.industry || "")}</p>
          ...
          <p><strong>最大リスク</strong> ${escapeHtml(item.max_risk || "")}</p>
          <p><strong>いま見る意味</strong> ${escapeHtml(item.appeal || "")}</p>
        </article>`;
      })
      .join("");
  }
